...
//public class MulticastOutputEmitter implements ChannelSelector<SerializationDelegate<MulticastMessage>> {
public class MulticastOutputEmitter implements MulticastChannelSelector {
	...
	private final TypeComparator<MulticastMessage> comparator;	// the comparator for hashing / sorting
	 ...
	 */
	public MulticastOutputEmitter(ShipStrategyType strategy) {
		this(strategy, null);
	}	
	 ...
	 */
	public MulticastOutputEmitter(ShipStrategyType strategy, TypeComparator<MulticastMessage> comparator) {
	 ...
	 */
	public MulticastOutputEmitter(ShipStrategyType strategy, TypeComparator<MulticastMessage> comparator, DataDistribution distr) {
		if (strategy == null) { 
	...
	@Override
	public final int[] selectChannels(SerializationDelegate<MulticastMessage> record, int numberOfChannels) {
		switch (strategy) {
		case FORWARD:
		case PARTITION_RANDOM:
	...
	private final int[] hashPartitionDefault(MulticastMessage record, int numberOfChannels) {
	...
	private final int[] rangePartition(MulticastMessage record, int numberOfChannels) {

...
/**
 * An input gate consumes one or more partitions of a *single* produced intermediate result. There
 * is one input channel at the input gate for each consumed partition.
 *
 * <pre>
 *
 * +-----+               +-------------+    +----------------+ <=======+=== requests = | Reduce 1 |
 * | Map | = produces => | Partition 1 | => | Subpartition 1 |     +----------+
 * +-----+               +-------------+    +----------------+
 *                                              +----------------+    |    |
 *                                                                    |    |
 *                                              +----------------+    |    |
 ...
 * +-------+               +-------------+  +=> | Subpartition 2 | <==+======== requests = | Reduce 2 |
 *                                              +----------------+                         +----------+
 * </pre>
 *
 *
 ...
 * <pre>
 *                        INTERMEDIATE RESULT   +----------------+                         +----------+
 * +-------+               +-------------+  +=> | Subpartition 1 | <=======+=== requests = | Reduce 1 |
 ...
 * </pre>
 *
 * In the above example, the map tasks produce a single intermediate result in parallel, resulting
 * in two partitions (Partition 1 and 2). For each consumed intermediate result, there is a single
 * input gate at each receiving task (Reduce 1 and 2).
 *
 * In the above example, the each Reduce is consuming a
 */
public class InputGate {
	...
	/**
	 * The total number of input channels. This is equivalent to the number of consumed
	 * partitions.
	 */
	private final int numberOfInputChannels;

	/** The invokable task this input gate belongs to. */
	...
	/** Buffer pool for incoming buffers. Incoming data is copied to buffers from this pool. */
	...
	/**
	 * Registered listener to forward channel notifications to.
	 */
	private volatile EventListener<InputGate> registeredListener;

	public InputGate(IntermediateDataSetID consumedResultId, int consumedSubpartitionIndex, int numberOfInputChannels, AbstractInvokable owner) {
	...
	IntermediateResultPartitionProvider getIntermediateResultPartitionProvider() {
		return owner.getEnvironment().getNetworkEnvironment().getPartitionManager();
	}

	TaskEventDispatcher getTaskEventDispatcher() {
		return owner.getEnvironment().getNetworkEnvironment().getTaskEventDispatcher();
	}

	ConnectionManager getConnectionManager() {
		return owner.getEnvironment().getNetworkEnvironment().getConnectionManager();
	}
	...
	public void closeInputChannel(int index) throws IOException {
		for (InputChannel ch : inputChannels.values()) {
			if (ch.getChannelIndex() == index) {
	...
	public void onAvailableData(InputChannel channel) {
	...
	public static InputGate create(AbstractInvokable owner, NetworkEnvironment networkEnvironment, PartitionConsumerDeploymentDescriptor desc) {
		...
		final InputGate reader = new InputGate(resultId, queueIndex, numberOfInputChannels, owner);

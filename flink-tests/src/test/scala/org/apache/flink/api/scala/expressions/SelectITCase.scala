...
import org.apache.flink.api.common.expressions.ExpressionException
...
class SelectITCase(mode: ExecutionMode) extends MultipleProgramsTestBase(mode) {
  ...
  def testSimpleSelectAll: Unit = {
    ...
    val ds = CollectionDataSets.get3TupleDataSet(env).select('_1, '_2, '_3)
  ...
  def testSimpleSelectWithNaming: Unit = {
    ...
    val env = ExecutionEnvironment.getExecutionEnvironment
    val ds = CollectionDataSets.get3TupleDataSet(env).select('_1 as 'a, '_2 as 'b).select('a, 'b)

...
public class SpargelPageRankComputer implements Serializable {
	...
	public DataSet<Tuple2<Long, Double>> computePageRank(
		...
		iteration.addBroadcastSetForUpdateFunction(PageRankUtil.NUMOFPAGES, numOfPages);
		
		DoubleSumAggregator agg = new DoubleSumAggregator();
		iteration.registerAggregator(PageRankUtil.VALUE_COLLECTED_BY_SINKS, agg);

		DoubleMaxAggregator maxRankChange = new DoubleMaxAggregator();
		iteration.registerAggregator(PageRankUtil.MAX_RANK_CHANGE, maxRankChange);

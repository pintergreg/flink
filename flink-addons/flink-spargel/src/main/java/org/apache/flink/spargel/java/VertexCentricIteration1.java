...
public class VertexCentricIteration1<VertexKey extends Comparable<VertexKey>, VertexValue, Message, EdgeValue>
	...
	private VertexCentricIteration1(
		...
		// check that the edges are actually a valid tuple set of vertex key
		// types
		TypeInformation<Tuple3<VertexKey, VertexKey, EdgeValue>> edgesType = edgesWithValue
				.getType();
		Validate.isTrue(edgesType.isTupleType() && edgesType.getArity() == 3,
				"The edges data set (for edges with edge values) must consist of 3-tuples.");
	...
	private TypeInformation<MessageWithHeader> getMessageType(
		...
		TypeInformation<VertexKey> keyType = TypeExtractor.createTypeInfo(
				MessagingFunction1.class, mf.getClass(), 1, null, null);
	...
	public DataSet<Tuple2<VertexKey, VertexValue>> createResult() {
		...
		CoGroupOperator<?, ?, Tuple2<VertexKey, VertexValue>> updates = messages1
	...
	/**
	 * Creates a new vertex-centric iteration operator for graphs where the
	 * edges are associated with a value (such as a weight or distance).
	 * 
	 * @param edgesWithValue
	 *            The data set containing edges. Edges are represented as
	 *            2-tuples: (source-id, target-id)
	 * @param uf
	 *            The function that updates the state of the vertices from the

	...
	implements CustomUnaryOperation<Tuple2<VertexKey, VertexValue>, Tuple2<VertexKey, VertexValue>>
{
	...
	private  VertexCentricIteration2(VertexUpdateFunction<VertexKey, VertexValue,  Message> uf,
			...
			int maximumNumberOfIterations)
	{
		...
		this.unPackedMessageType = TypeExtractor.createTypeInfo(MessagingFunction2.class, mf.getClass(), 2, null, null);
		//this.messageType = mf.getMessageType();
	}
	
	
	@SuppressWarnings("rawtypes")
	private TypeInformation<MessageWithHeader> getMessageType(MessagingFunction2<VertexKey, VertexValue, Message, EdgeValue> mf) {
...
//		DataSet<Tuple2<VertexKey, MessageWithSender<VertexKey, Message>>> messages;
		...
		CoGroupOperator<?, ?, Tuple2<VertexKey, VertexValue>> updates =
				messages2.coGroup(iteration.getSolutionSet()).where(0).equalTo(0).with(updateUdf);
	...
	implements JoinFunction<Tuple2<VertexKey,VertexKey>, 
		Tuple2<VertexKey, MessageWithHeader<VertexKey,Message>>, Tuple2<VertexKey, Message>>,
	...
	implements JoinFunction<Tuple3<VertexKey,VertexKey, Integer>, 
		Tuple2<VertexKey, MessageWithHeader<VertexKey,Message>>, Tuple2<VertexKey, Message>>,
		ResultTypeQueryable<Tuple2<VertexKey, Message>>{
		...
		private UnpackMessage2(TypeInformation<Tuple2<VertexKey, Message>> resultType)
		{
			this.resultType = resultType;
		}
						...
						int maximumNumberOfIterations)
	{
		...
		MessagingFunction2<VertexKey, VertexValue, Message, Object> tmf = 
								(MessagingFunction2<VertexKey, VertexValue, Message, Object>) messagingFunction;
		
		return new VertexCentricIteration2<VertexKey, VertexValue, Message, Object>(vertexUpdateFunction, tmf, edgesWithoutValue, maximumNumberOfIterations);
	}
	...
	implements ResultTypeQueryable<Tuple2<VertexKey, MessageWithHeader<VertexKey, Message>>>
	{
		...
		private MessagingUdfWithEdgeValues(MessagingFunction2<VertexKey, VertexValue, Message, EdgeValue> messagingFunction,
				TypeInformation<Tuple2<VertexKey, MessageWithHeader<VertexKey, Message>>>  resultType)
		{
			this.messagingFunction = messagingFunction;
			this.resultType = resultType;

...
public class FaultToleranceWithTopologyTest {
	...
	public static class MockLocalStreamEnvironment extends LocalStreamEnvironment {

		public MockLocalStreamEnvironment() {
			this.jobGraphBuilder = new MockJobGraphBuilder();
		}
	}

	public static class MockJobGraphBuilder extends JobGraphBuilder {
		private static final Logger LOG = LoggerFactory.getLogger(MockJobGraphBuilder.class);
	...
	public static class MockStreamTaskCollector<T> extends StreamTaskCollector<T> {
		...
		@Override
		protected void emitToOutputs(SerializationDelegate<StreamRecord<T>> outRecord) {
	...
	public static class MockFTLayer extends FTLayer {
		...
		public void fail(long sourceRecordId) {
			if (ackerTable.contains(sourceRecordId)) {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug("Source record id {} has been failed",
							Long.toHexString(sourceRecordId));
	...
	public void test() {
		...
		final MockLocalStreamEnvironment env = new MockLocalStreamEnvironment();

...
import org.apache.flink.api.common.expressions.Literal
  ...
  def main(args: Array[String]) {
    ...
    val adjacencyLists = links
      .groupBy("sourceId").reduceGroup( new GroupReduceFunction[Link, AdjacencyList] {
        ...
        override def reduce(
            values: java.lang.Iterable[Link],
    ...
    val finalRanks = pagesWithRanks.iterateWithTermination(maxIterations) {
      currentRanks =>
        val newRanks = currentRanks
          ...
          .group('pageId).select('pageId, 'rank.sum as 'rank)
        ...
        val termination = currentRanks.join(newRanks)
          .where('left$pageId === 'right$pageId && ('left$rank - 'right$rank).abs > EPSILON)

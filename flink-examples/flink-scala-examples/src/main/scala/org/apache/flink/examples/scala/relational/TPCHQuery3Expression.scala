...
object TPCHQuery3Expression {
  ...
  def main(args: Array[String]) {
    ...
    val lineitems = getLineitemDataSet(env)
      ...
      .select('id, 'extdPrice, 'discount, 'shipDate)

    val customers = getCustomerDataSet(env)
      .select('id, 'mktSegment)
      .filter( 'mktSegment === "AUTOMOBILE" )

    val orders = getOrdersDataSet(env)
      .filter( o => dateFormat.parse(o.orderDate).before(date) )
      .select('orderId, 'custId, 'orderDate, 'shipPrio)

    val items = orders.join(customers).where('custId === 'id)
      .select('orderId, 'orderDate, 'shipPrio)
      .join(lineitems).where('orderId === 'id)
      .select('orderId, 'extdPrice * (Literal(1.0f) - 'discount) as 'revenue, 'orderDate, 'shipPrio)

    val result = items
      .group('orderId, 'orderDate, 'shipPrio)

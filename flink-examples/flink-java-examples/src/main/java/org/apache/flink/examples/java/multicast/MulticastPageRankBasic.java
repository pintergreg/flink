 ...
 * For example <code>"1\n2\n12\n42\n63\n"</code> gives five pages with IDs 1, 2,
 * 12, 42, and 63.
...
public class MulticastPageRankBasic {
...
//				.createRemoteEnvironment("127.0.0.1", 6123, pathToJar);
		...
		DataSet<Tuple2<Long, Long[]>> adjacencyListInput = linksInput
				.groupBy(0).reduceGroup(new BuildOutgoingEdgeList());
...
//				.map(new Dampener(DAMPENING_FACTOR, numPages));
		...
		// execute program
		env.execute("Multicast Basic Page Rank Example");
	...
	public static final class BuildOutgoingEdgeList implements
		...
		public void reduce(Iterable<Tuple2<Long, Long>> values,
			...
			out.collect(new Tuple2<Long, Long[]>(id, neighbors
					.toArray(new Long[neighbors.size()])));
		}
	}
	...
	public static final class JoinVertexWithEdgesMatch
			implements
			FlatMapFunction<Tuple2<Tuple2<Long, Double>, Tuple2<Long, Long[]>>, MulticastMessage> {

		@Override
		public void flatMap(
				Tuple2<Tuple2<Long, Double>, Tuple2<Long, Long[]>> value,
				Collector<MulticastMessage> out) {
			Long[] neigbors = value.f1.f1;
			long[] formattedNeighbours = new long[neigbors.length];
			int index = 0;
			for (Long i : neigbors) {
				formattedNeighbours[index] = i;
				index++;
			}
			...
			out.collect(new MulticastMessage(formattedNeighbours,
					rankToDistribute));
		}
	}
	...
	private static boolean parseParameters(String[] args) {
		...
		if (args.length > 0) {
			if (args.length == 5) {
				...
				System.err
						.println("Usage: PageRankBasic <pages path> <links path> <output path> <num pages> <num iterations>");
				return false;
			}
		} else {
			System.out
					.println("Executing PageRank Basic example with default parameters and built-in default data.");
			System.out
	...
	private static DataSet<Long> getPagesDataSet(ExecutionEnvironment env) {
		if (fileOutput) {
			return env.readCsvFile(pagesInputPath).fieldDelimiter(' ')
					.lineDelimiter("\n").types(Long.class)
	...
	private static DataSet<Tuple2<Long, Long>> getLinksDataSet(
		...
		if (fileOutput) {
			return env.readCsvFile(linksInputPath).fieldDelimiter(' ')

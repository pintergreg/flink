...
import org.apache.flink.api.common.expressions.FieldAccessor
...
/** Base class for all code generation classes. This provides the functionality for generating
  ...
  *           as "(IN1, IN2) => OUT".
  */
abstract class ExpressionCodeGenerator[R](val nullCheck: Boolean = false, cl: ClassLoader) {
  ...
  protected val toolBox = ReflectionLock.synchronized { runtimeMirror(cl).mkToolBox() }
...
//  protected def generateExpression(expr: Expression): GeneratedExpression = {
    ...
    val code: Seq[Tree] = cleanedExpr match {
      ...
      case FieldReference(elementType, inputIndex, _, _, fieldType, accessor) =>
        val fieldCode = getField(newTermName("input" + inputIndex), fieldType, accessor)
        if (nullCheck) {
          q"""
  ...
  protected def typeForTypeInfo(tpe: TypeInformation[_]): Type = tpe match {
    ...
    case _ =>
      val mirror = runtimeMirror(tpe.getTypeClass.getClassLoader)
      val clazz = mirror.staticClass(tpe.getTypeClass.getCanonicalName).selfType
